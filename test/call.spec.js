...
describe('calls', function () {
    describe('return simple results', () => {
        ...
        it('return param async', () => {
            return service.callInternal('test.returnParamAsync', 'hello').then(o => {
                ...
                expect(o).property('time').a('number').above(497);
            });
        });
    });
    ...
    describe('passage', () => {
        it('with one child', () => {
            return service.callInternal('test.passageWithOneChild').then(o => {
                ...
                expect(o).property('time').a('number');
                ...
                expect(child).property('time').a('number');
            });
        });

        it('with one child returns error', () => {
            return service.callInternal('test.passageWithOneChildReturnsError')
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('childs').an('array').length(1);
        ...
        it('async with 4 childs and 1 error', () => {
            return service.callInternal('test.asyncPassageWith4ChildsAnd1Error').then(o => {
                ...
                expect(o).property('time').a('number').above(57);
        ...
        it('different childs begin time', () => {
            return service.callInternal('test.asyncPassageWith4ChildsAnd1ErrorWithDifBegin').then(o => {
                ...
                expect(o).property('time').a('number').above(397);

                const first = o.childs[0];
                expect(first).property('name').eql('test.asyncString0');
                expect(first).property('result').eql('0');
                expect(first).property('time').a('number').above(47);
                expect(first).property('start').a('number').above(97);

                const second = o.childs[1];
                expect(second).property('name').eql('test.asyncString1');
                expect(second).property('result').eql('1');
                expect(second).property('time').a('number').above(57);
                expect(second).property('start').a('number').above(197);

                const third = o.childs[2];
                expect(third).property('name').eql('test.asyncString2');
                expect(third).property('result').eql('2');
                expect(third).property('time').a('number').above(39);
                expect(third).property('start').a('number').above(297);

                const error = o.childs[3];
                expect(error).property('error').property('message').contain('is not a function');
                expect(error).property('start').a('number').above(397);
    ...
    describe('return error', () => {
        it('string is not a function', () => {
            return service.callInternal('test.throwError1', 'qwe')
                .then(assert.fail)
                .catch(o => {
        ...
        it('throw number', () => {
            return service.callInternal('test.throwError2')
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('error');
        ...
        it('throw string', () => {
            return (service.callInternal('test.throwError3'))
                .then(assert.fail)
                .catch(o => {
        ...
        it('throw new error', () => {
            return service.callInternal('test.throwError4')
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('error').property('message').eql('123');
        ...
        it('throw new ServiceError', () => {
            return service.callInternal('test.throwError5')
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('error').property('message').eql('123');
                    expect(o.error).property('trace').an('array').property(0)
        ...
        it('throw async error in call callback', () => {
            return service.callInternal('test.throwAsyncErrorInCallCallback')
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('childs').an('array').length(1);
                    expect(o).property('error');
        ...
        it('undefined method', () => {
            return service.callInternal('test.undefinedMethod')
                .then(assert.fail)
                .catch(o => {
        ...
        it('undefined module', () => {
            return service.call('undefinedModule.undefined')
                .then(assert.fail)
                .catch(o => {
        ...
        it('call with circle param', () => {
            let x = {};
            x.x = x;
            return service.callInternal('test.returnParamsWithAddValue', x)
                .then(assert.fail)
                .catch(o => {
                    expect(o).property('error');
        ...
        it('call returns circular result', () => {
            return service.callInternal('test.returnCircular', 'some_param')
                .then(assert.fail)
                .catch(o => {
        ...
        it('request calls count exceed', () => {
            return service.call('test.circle1')
                .then(assert.fail)
                .catch(error => {
    ...
    describe('modules cooperation', () => {
        ...
        it('method in other module returns error', () => {
            return service.call('test1.callPublicMethodReturnsError')
                .then(assert.fail)
                .catch(error => {
                    expect(error).property('message').eql('123');
                    expect(error).property('trace').an('array')
        ...
        it('check to accessible method', () => {
            return service.callInternal('test1.checkToAccessible').then(o => {
                ...
                expect(result[2]).an('object').property('details')
                    .eql('Method is not part of the API: test.privateMethod');
