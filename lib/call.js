...
module.exports = class Call {
    ...
    call(name, params) {
        ...
        if (name.indexOf('.') === -1) {
            ...
            let moduleMethodNameObj = this.service.constructor.parseModuleMethod(name);
        ...
        if (isNearby) {
            ...
            method = this.service.getModuleAndMethod(moduleName + '.' + methodName).method;
        }

        let call = new Call(method, params, this.request);
        ...
        return (new Promise((resolve, reject)  => {
            if (!isNearby && !method.isApi && !method.isPublic) {
                reject(this.formalizeResult(undefined, new Error({
                    code: 405,
