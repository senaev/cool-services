...
module.exports = class Call {
                ...
                request.post({
                    ...
                    if (error) {
                        ...
                        if (response.statusCode == 200 || response.statusCode == 500) {
                            this.externalParams = body.external;
                            this.time = body.time;
                            this.externalTime = Date.now() - this.timeBegin;
     ...
     */
    callMethod(params) {
        this.method.meetExpectation(params);
    ...
    toObject() {
        ...
        if (this.isExternal) {
            ...
            if (this.externalParams) {
                this.externalParams.unshift(externalObj);
                callResult.external = this.externalParams;
            } else {
                callResult.external = [externalObj];
            }

            callResult.time = this.time;
        } else {
            callResult.time = Date.now() - this.timeBegin;
        }
